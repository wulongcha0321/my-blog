...
.adv-page
  .article-list
    .article-item(v-for="(item,index) in blogList" :key="item.id")
      .feed-body
        .feed-header
  ...
  methods: {
    fetchBlogList() {
      getAdvertisementList()
        ...
        .catch((err) => {
          console.log("fetchblogList error", err);
        });
    },
    handleLike(item) {
      if (item.like === 1) {
        ...
        cancelLike({
          ...
          .then((res) => {
            if (res.code === 200) {
              this.blogList.map((v, index) => {
                if (v.id === item.id) {
                  this.blogList[index].like = 0;
                  this.blogList[index].like_count--;
                }
              });
            } else {
              this.$message.error(res.message);
    ...
    handleComment(item) {
      this.blogList.map((v, index) => {
        if (v.id === item.id) {
          this.blogList[index].commentVisible =
            !this.blogList[index].commentVisible;
          this.blogList[index].newComment = "";
        }
      });
    },
    // 提交评论
